...
const ui = {
  ...
  topOutputDeviceSelect: document.getElementById('topOutputDeviceSelect'),
  topOutputDeviceHint: document.getElementById('topOutputDeviceHint'),
  ...
  widthScopeDot: document.getElementById('widthScopeDot'),
...
function bindUiEvents() {
  ...
  ui.btnBypass.addEventListener('click', async () => {
    ...
    ui.btnBypass.classList.toggle('active', next);
    renderAll();
  ...
  ui.topOutputDeviceSelect?.addEventListener('change', async () => {
    const option = ui.topOutputDeviceSelect.selectedOptions?.[0];
    await applyStudioOutputDevice({
      deviceId: option?.value || 'default',
      label: option?.dataset?.label || option?.textContent || 'System Default'
    }, ui.topOutputDeviceHint).catch((error) => { if (ui.topOutputDeviceHint) ui.topOutputDeviceHint.textContent = error.message || 'Unable to route output.'; });
  });
...
function renderChromeState() {
  ...
  ui.sourceChip.classList.toggle('active', isActive);
  ui.sourceTitle.textContent = state.sourceTitle || 'No active capture';
  ui.startStopButton.textContent = isActive ? 'Stop Enhance' : 'Start Enhance';
  ui.startStopButton.classList.toggle('danger', isActive);
  ui.btnBypass.classList.toggle('active', Boolean(state.output?.bypass));
  updateRackState();
}
...
function buildSkeleton() {
  ...
  layers.specInStroke.setAttribute('class', 'spec-stroke-in');
  svg.appendChild(layers.specInStroke);
  layers.specOut = document.createElementNS(SVG_NS, 'path');
  layers.specOut.setAttribute('class', 'spec-stroke-out');
...
function renderPresetDropdowns() {
  ...
  populateTopOutputDevices().catch((error) => { if (ui.topOutputDeviceHint) ui.topOutputDeviceHint.textContent = error.message || 'Unable to list output devices.'; });
}
...
function renderModulePresetSelect(key, select) {
  ...
  if (select.dataset.optionIds !== desired) {
    ...
    manual.value = '';
    manual.textContent = 'Custom / Manual';
...
function renderColorControls() {
  ...
  mode.innerHTML = ['clean', 'warm', 'modern'].map((item) => `<button type="button" class="${state.color.mode === item ? 'on' : ''}" data-mode="${item}">${item}</button>`).join('');
...
function drawColorViz() {
  ...
  const intensity = on ? clamp(drive * 0.65 + harm * 0.5 + liveAmount * 0.25 + 0.08, 0.05, 1) : 0.05;
  ...
  ctx.font = '8px "Sometype Mono", ui-monospace, monospace';
...
function renderWidthControls() {
  ...
  state.width = { ...DEFAULT_WIDTH, ...(state.width || {}) };
  ui.widthControls.innerHTML = '';
  const controls = [
    ['width', 'Width', 0, 150, 1, '%'],
    ['monoBassFreq', 'Mono Bass', 60, 250, 1, 'Hz'],
    ['sideTone', 'Side Air', -12, 12, 0.5, 'dB']
  ];
  for (const [field, label, min, max, step, unit] of controls) {
...
function updateRackState() {
  ...
  document.querySelectorAll('.rack-node[data-jump]').forEach((node) => {
    ...
    node.classList.toggle('bypassed', !on);
...
function updateModuleBypassButtons(moduleStates = null) {
  ...
  for (const [key, button, card] of configs) {
    ...
    if (button) {
      ...
      button.classList.toggle('is-bypass', !on);
      button.textContent = on ? 'ON' : 'BYPASS';
      button.setAttribute('aria-pressed', String(on));
      button.title = on ? 'Click to bypass this module' : 'Click to enable this module';
    }
    card?.classList.toggle('is-bypassed', !on);
  }
}
...
function renderOutputControls() {
  ...
  for (const [field, label, min, max, step, unit] of controls) {
    ui.outputControls.appendChild(createSliderControl({
      label,
...
function createOutputDeviceControl() {
  ...
  wrapper.className = 'device-control';
  wrapper.innerHTML = `
    <div class="device-control-top">
      <label>
        <span>Output device</span>
        <select aria-label="Output audio device"></select>
      </label>
    </div>
    <small data-device-hint>Auto-detecting output devices…</small>`;
  const select = wrapper.querySelector('select');
  const hint = wrapper.querySelector('[data-device-hint]');
  select.addEventListener('change', async () => {
    const option = select.selectedOptions?.[0];
    await applyStudioOutputDevice({
      deviceId: option?.value || 'default',
      label: option?.dataset?.label || option?.textContent || 'System Default'
    }, hint).catch((error) => { hint.textContent = error.message || 'Unable to route output.'; });
  });
  populateStudioOutputDevices(select, hint).catch((error) => {
    hint.textContent = error.message || 'Unable to refresh output devices.';
  });
  return wrapper;
}


async function populateTopOutputDevices(preferredDeviceId = null) {
  if (!ui.topOutputDeviceSelect || !ui.topOutputDeviceHint || !state) return;
  const select = ui.topOutputDeviceSelect;
  const hint = ui.topOutputDeviceHint;
  const selectedId = normalizeOutputDeviceId(preferredDeviceId || state.output?.outputDeviceId || 'default');
  hint.textContent = 'Auto-detecting…';
  ...
  updateOutputDeviceHint(hint);
}

async function populateStudioOutputDevices(select, hint, preferredDeviceId = null) {
  const selectedId = normalizeOutputDeviceId(preferredDeviceId || state.output?.outputDeviceId || 'default');
  hint.textContent = 'Auto-detecting output devices…';
  const detection = await detectAudioOutputDevices();
  const devices = detection.devices || [];
  select.innerHTML = '';
  for (const device of devices) {
  ...
  if (detection.nonDefaultCount === 0) {
    hint.textContent = 'Auto-detect: System Default only. Check USB/VB-Cable in Windows Sound.';
    return;
  }
  updateOutputDeviceHint(hint);
}

async function applyStudioOutputDevice(device, hint) {
  ...
  state.output = { ...state.output, outputDeviceId, outputDeviceLabel };
  updateOutputDeviceHint(hint);
  ...
  updateOutputDeviceHint(hint);
  populateTopOutputDevices(outputDeviceId).catch(() => {});
}

function updateOutputDeviceHint(hint) {
  if (!hint) return;
  ...
  const issue = status === 'failed' || status === 'unsupported' || status === 'playback-blocked';
  const routed = status === 'routed';
  hint.textContent = `${issue ? 'Route issue' : routed ? 'Routed' : 'Output'}: ${label}`;
}
  ...
  wrap.innerHTML = `
    ...
    <div class="knob-dial" tabindex="0" role="slider" aria-label="${label}"
      ...
      <svg viewBox="0 0 56 56">
        <defs><radialGradient id="kc${uid}" cx="38%" cy="30%" r="78%">
          <stop offset="0%" stop-color="#232B3C"/><stop offset="58%" stop-color="#10141D"/><stop offset="100%" stop-color="#090C12"/>
        </radialGradient></defs>
...
function drawCompressorCurve() {
  ...
  context.font = '9px "Sometype Mono", ui-monospace, monospace';
...
function toggleAB() {
  ...
  if (slotB === null) {
    slotB = current;
    onSlotA = false;
    ui.btnAB.textContent = 'A saved';
    ui.btnAB.classList.add('active');
    return;
  }
  const tmp = slotB;
  slotB = current;
  restoreAudioOnly(tmp);
  onSlotA = !onSlotA;
  ui.btnAB.textContent = onSlotA ? 'A·/B' : 'A/·B';
  renderAll();
  commitAndSend();
}
...
function snapshotAudioOnly() {
  return JSON.stringify({ eqEnabled: state.eqEnabled !== false, eq: serializeBands(), compressor: state.compressor, output: state.output, color: state.color, width: state.width });
}
...
function restoreAudioOnly(serialized) {
  ...
  updateEngineState({ eqEnabled: state.eqEnabled !== false, eq: serializeBands(), compressor: state.compressor, output: state.output, color: state.color, width: state.width }).catch(console.error);
  renderCompressorControls();
  renderColorControls();
  renderWidthControls();
  renderOutputControls();
...
function updateMeters(meters) {
  ...
  ui.inputMeter.value = inputPeak;
  ui.outputMeter.value = outputPeak;
  ...
  ui.gainReductionBar.style.transform = `scaleX(${grScale})`;
  if (ui.compressorReductionValue) {
    ui.compressorReductionValue.textContent = `${displayedCompressorReduction.toFixed(1)} dB`;
  ...
  if (ui.widthScopeDot) {
    const amount = state?.width?.enabled ? clamp((state.width.width - 100) / 50, -1, 1) : 0;
    ui.widthScopeDot.style.transform = `translate(${amount * 34}px, ${-displayedCorrelation * 8}px)`;
  }
  if (ui.smartGainChip) {
...
function smoothPeak(previous, next) {
  ...
  const alpha = target > previous ? 0.55 : 0.18;
...
const RTA_ATTACK_MS = 50;
const RTA_RELEASE_MS = 460;
const RTA_IDLE_ATTACK_MS = 120;
const RTA_IDLE_RELEASE_MS = 760;
...
function tickSpectrum(ts) {
  ...
  if (hasLive) {
    ...
    if (targetKey !== lastRtaTargetKey) {
      ...
      shapeSpectrumTarget(specInRawTarget, specInTarget);
      shapeSpectrumTarget(specOutRawTarget, specOutTarget);
  ...
  for (let i = 0; i < SPEC_N; i += 1) {
    specInVal[i] = smoothDbVisual(
      ...
      specInTarget[i],
      deltaMs,
      hasLive ? RTA_ATTACK_MS : RTA_IDLE_ATTACK_MS,
      hasLive ? RTA_RELEASE_MS : RTA_IDLE_RELEASE_MS
    );
